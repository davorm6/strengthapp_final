...
class ExerciseListActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val navigationView = findViewById<NavigationView>(R.id.nav_view_exercise_list)
        ...
        val toggle = ActionBarDrawerToggle(
            this, drawer_layout_exercise_list, toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close
        )
        drawer_layout_exercise_list.addDrawerListener(toggle)
        toggle.syncState()


        if(SharedPreferenceManager.getTrainerID(applicationContext) == 0) {
            nav_view_exercise_list.menu.findItem(R.id.trainer_profile).setVisible(false)
        }
        else {
            nav_view_exercise_list.menu.findItem(R.id.trainer_profile).setVisible(true)
        }

        if(!SharedPreferenceManager.getAdmin(applicationContext)) {
            nav_view_exercise_list.menu.findItem(R.id.admin).setVisible(false)
        }
        else {
            nav_view_exercise_list.menu.findItem(R.id.admin).isVisible = true
        }

        nav_view_exercise_list.setNavigationItemSelectedListener(this)

        adapter = ExerciseAdapter(this.baseContext)

        rv_elist_item.layoutManager = LinearLayoutManager(this)
        rv_elist_item.adapter = adapter

        addrequest.setOnClickListener { v ->
            val intent = Intent(applicationContext, ExerciseRequestActivity::class.java)
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        menuInflater.inflate(R.menu.exercise_list, menu)
        return true
    }
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        ...
        drawer_layout_exercise_list.closeDrawer(GravityCompat.START)
        return true
    }
}
